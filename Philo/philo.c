...
/*   Updated: 2024/10/08 23:58:11 by codespace        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_clean(t_info *info)
{
	...
	{
		pthread_mutex_destroy(&info->philos[i].time_mtx);
		pthread_mutex_destroy(&info->forks[i]);
		i++;
	}
	pthread_mutex_destroy(&info->meals_mtx);
	pthread_mutex_destroy(&info->simul_mtx);
...
int	main(int ac, char **av)
{
	...
	while (i < info.num_of_philos)
	{
		if (pthread_create(&info.philos[i].ptid, NULL, activity,
				&info.philos[i]))
